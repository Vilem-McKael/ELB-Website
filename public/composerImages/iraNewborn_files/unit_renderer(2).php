var jvx_6656684427dbbNonMraidTagInMraidEnv = false;
var jvx_6656684427dbbdyPropsACK = false;
...
(function(){
	if(typeof MRAID_ENV != 'undefined'){
		...
		jvx_6656684427dbbNonMraidTagInMraidEnv = true;
	}
})();

if(typeof jvxObjectList == "undefined"){
	var jvxObjectList = ["jvx_6656684427dbb"];
}else{
	jvxObjectList.push("jvx_6656684427dbb");
}
    ...
    try{
        ...
        if(cScrNode == undefined){
            ...
            for(var i=0;i<scrNodes.length;i++){
                ...
                if(scrSrc &&  scrSrc.indexOf("https://as.jivox.com/unit/unit_renderer.php?cbName=__IntegralAS_b561874ffc254b74eaab2e944b58d4f6_4833&es_pId=3cbb2b9c&showAdChoices=0&isDynamic=1&campaignId=177388&ts_pId=3cbb2b9c&siteId=2637e5c7c09080&dspId=a7b0f97e-f637-4081-9190-a5ae87cce1a9&bDim=320x50&creativeUnitType=20&jvxVer=2&bUnitId=2002&us_privacy=$%7BUS_PRIVACY%7D&gdpr_consent=$%7BGDPR_CONSENT_294%7D&gdpr=$%7BGDPR%7D&r=%7B%25random%7D&cMacro=%7B%25short_redirect%7D&vfwctx=eyJjIjoiYWxsLTEifQ&gdpr=0&gdpr_consent=&gdpr_pd=1&format=jsonp") != -1){
                    cScrNode = scrNodes[i];
                }
            }
        }   
...
function createiFrameNode(obj){
	...
	if(document.body == null){
		document.write("<div id='jvxSkeletonRef_6656684427dbb' style='display:none;'></div>")
	}
...
function replaceRunTimeMacros( trackerUrl, params){
	try{
		...
		runtimeMacroValues['ah'] = (jvx_6656684427dbb.baseHeight) ? jvx_6656684427dbb.baseHeight : (params['clientHeight'] ? params['clientHeight'] : "") ;
		runtimeMacroValues['aw'] =  jvx_6656684427dbb.baseWidth ? jvx_6656684427dbb.baseWidth : (params['clientWidth'] ? params['clientWidth'] : "");
		var tv = runtimeMacroValues['ct'] - jvx_6656684427dbb.layoutLoadedTime;
...
*/
var jvx_6656684427dbbTraffickedInFrame = false,			// COM: should be true, if !busted and self != top
	jvx_6656684427dbbSafeFrameSupported = false,		// COM: should be true only when safeFrame is supported in the page
	jvx_6656684427dbbCanBustFromFrame = true,			// COM: should be false, when all busting method fails
	jvx_6656684427dbbInNestedFrame = false;				// COM: should be true, when tag trafficked inside nested frame

if(self != top){
	jvx_6656684427dbbTraffickedInFrame = true;
}

try{
		$sf.ext.register(320, 50);
		jvx_6656684427dbbSafeFrameSupported = true;
}catch(e){
	jvx_6656684427dbbSafeFrameSupported = false;
}
...
var adViewabilityDetectInit = function (visibilityThreshold) { 
    try {
        ...
        boxElement = document.querySelector('#adUnitContainer_6656684427dbb');
                    ...
                    if (!adInVPForOneSec && !timerRunning) {
                        ...
                        timeoutHandle = setTimeout(function () {
                            adInVPForOneSec = true;
                                                        if(typeof jvx_6656684427dbb != "undefined")
                            jvx_6656684427dbb.recordEvent(73);
                            else 
                            recordEvent(73);
                                                    }, 1000);
                    }
...
var CreativeUnit = (function(){
	...
	function receiveHtml5Message(e){
        ...
        var s = e.data.split(':', 10);
		        if(s && s.length >= 2 && s[0] == 'jvx_6656684427dbb'){
			var msg = s[1];
			if(msg == 'openExpansion'){
									jvx_6656684427dbb.openClickThrough();
							} else if(msg == 'closeExpansion'){
				jvx_6656684427dbb.closeExpansion(false, false);
			} else if(msg == 'expansionLoaded'){
							} else if(msg == 'recordUIR'){
				jvx_6656684427dbb.UIRRecorded = true;
			} else if(msg == 'lastMouseLeave'){
				jvx_6656684427dbb.lastMLeaveId = s[2];
			} else if(msg == 'clearUIR'){
				clearTimeout(jvx_6656684427dbb.UIRTimer);
			} else if(msg == 'recordDwell'){
				if(!jvx_6656684427dbb.DwellRecorded){
					jvx_6656684427dbb.recordDwellEvent(49, jvx_6656684427dbb.DwellElapsedTime);
				}
			} 
												else if(msg=="dynamicProperties") {
                jvx_6656684427dbb.updateDYProperties(s[2],1);
							}
			else if(msg=="clickThroughUrl") { 
								jvx_6656684427dbb.clickTagURL = s.slice(2).flat(1).join(":");
			}
			else if(msg == "clickTrackers") {	
								jvx_6656684427dbb.clickTrackers =  s.slice(2).join(":").split(",").map(function(url){ return decodeURIComponent(url) });
				this.clickTrackers = jvx_6656684427dbb.clickTrackers;
			}
												                        else if(msg == "miniScrollerBgInfo"){
                var miniScrollerBgURL = decodeURIComponent(s[2]);
                jvx_6656684427dbb.fixedBgAstLeft = parseInt(s[3]);
                jvx_6656684427dbb.renderFixedPageBg(miniScrollerBgURL,s[3]);
            } 
		}
	}
	...
	function postHtml5Message(winRef, msg){
		try{
			winRef.postMessage('jvx_6656684427dbb' + ':' + msg, "*");
		}catch(e){
			if(typeof(winRef.contentWindow) != 'undefined')
				winRef.contentWindow.postMessage('jvx_6656684427dbb' + ':' + msg, "*");
		}
	}
	...
	*/
	...
	function clearNode(pNode, idPrefix){
        ...
        if(jvx_6656684427dbbTraffickedInFrame){
                        try{
...
finalURL = replaceCacheBuster4;			//finalURL = replaceCacheBuster;
			...
			if(url.indexOf("?") == -1){
				...
				finalURL = url + "&r=" + Math.random();
					...
					if(typeof jvx_6656684427dbb != 'undefined' && url.match("https://evs.jivox.com/jivox/serverAPIs/saveImpression.php")){	//Add es_cgName and es_segName params only for the evs trackers URL not for the thirdParty trackers.
				finalURL += "&es_cgName_"+jvx_6656684427dbb.DYReportingKey;
                if(jvx_6656684427dbb.DYSegmentName != ""){
                    finalURL += "&es_segName_"+jvx_6656684427dbb.DYSegmentName;
                }
			}
				imgEl.src = finalURL;
			}       
	...
	function setOpacity(myElement, opacityValue) {
		...
		var cLeft = sfLeft + "px",
			cTop = sfTop + "px",
			cNode = getElById(jvx_6656684427dbb.adUnitContainerId);
		addStyles(cNode, {"left":cLeft, "top":cTop});
	}
	function renderAdInsideSafeFrame(animDir){
		if(jvx_6656684427dbbSafeFrameSupported){
			/* ------------- SafeFrame Settings--------------- */
						var ref = jvx_6656684427dbb,
	 ...
	 /* var rect = obj.getBoundingClientRect();
		...
		var vpStyles = findPos(getElById(jvx_6656684427dbb.adUnitContainerId)),
			vpTop = "", vpLeft = "";
	...
	function mraidHandleOnViewable(viewable){
		jvxMraidIsShown = viewable;		 
			if(!jvxMraidIsShown || getElById(jvx_6656684427dbb.baseContainerId)){
				return; 
			}
		jvx_6656684427dbb.init();
	...
	function handleMraidViewableImprFiring(){
		if (!adInVPForOneSec && !timerRunning) {
			...
			var timeoutHandle = setTimeout(function () {
				adInVPForOneSec = true;
				jvx_6656684427dbb.recordEvent(74);
			}, 1000);
		}
	}
	...
	function CreativeUnit(uniqId){
	    ...
	    this.id = uniqId;  
		this.playerParamsMap = {"reportingURL":"https:\/\/evs.jivox.com","cbName":"__IntegralAS_b561874ffc254b74eaab2e944b58d4f6_4833","es_pId":"3cbb2b9c","showAdChoices":"0","isDynamic":"1","campaignId":"177388","ts_pId":"3cbb2b9c","siteId":"2637e5c7c09080","dspId":"a7b0f97e-f637-4081-9190-a5ae87cce1a9","bDim":"320x50","creativeUnitType":"20","jvxVer":"2","bUnitId":"2002","us_privacy":"${US_PRIVACY}","gdpr_consent":"","gdpr":"0","r":"{%random}","cMacro":"{%short_redirect}","vfwctx":"eyJjIjoiYWxsLTEifQ","gdpr_pd":"1","format":"jsonp","0":"","serverURL":"https:\/\/as.jivox.com","userHandlesBaseClick":"0"};
		this.campaignId = '177388';
		this.siteId = '2637e5c7c09080';
		this.placementId = '3cbb2b9c';
		...
		this.browserVersion = '125.0.0.0';
								this.clickTagURL = 'https://www.t-mobile.com/';
		this.clickTrackers = [];
		...
		this.baseURL = 'https://as.jivox.com/unit/layout_renderer.php?cbName=__IntegralAS_b561874ffc254b74eaab2e944b58d4f6_4833&es_pId=3cbb2b9c&showAdChoices=0&isDynamic=1&campaignId=177388&ts_pId=3cbb2b9c&siteId=2637e5c7c09080&dspId=a7b0f97e-f637-4081-9190-a5ae87cce1a9&bDim=320x50&creativeUnitType=20&jvxVer=2&bUnitId=2002&us_privacy=%24%7BUS_PRIVACY%7D&gdpr_consent=&gdpr=0&r=%7B%25random%7D&cMacro=%7B%25short_redirect%7D&vfwctx=eyJjIjoiYWxsLTEifQ&gdpr_pd=1&format=jsonp&objectName=jvx_6656684427dbb&adUnitId=2002&jvxSessionId=1716938820.8253';
		this.layoutLoadedTime = 0;
        			this.baseURL += '&base=1';
            this.baseURL += '&creativeResolveBeginTime='+Date.parse(creativeResolveBeginTime);
            							this.baseURL += '&omid=0';
						        this.baseBgType = 'none';
		this.baseBgURL = 'https://assets.jivox.com/assets/widgets/2024/4/a57332z66150579e23b9/1/TMO_320x50/index.html';
		this.baseBgColor = '';
        ...
        this.baseWidth = '320';
		this.baseHeight = '50';
                        this.origBaseWidth = '320';
        this.origBaseHeight = '50';
	...
	CreativeUnit.prototype = {
		constructor:CreativeUnit,
         ...
         sendCookieRequest: function(cookieName){
	        if(jvx_6656684427dbbSafeFrameSupported){
		         var callBackFnName ='jvx_6656684427dbb'+".updateDYProperties";
		         jvx_6656684427dbb.addScriptTagToBody('https://as.jivox.com'+"/unit/getCookies.php?cookieName="+cookieName+"&callback="+callBackFnName+"&deleteCookie="+1+"&r_="+Math.random());  
	         }                  
         },
         updateDYProperties: function(val,isNotCookieData){
            ...
            jvx_6656684427dbb.DYReportingKey = dynPropArr[0] ? decodeURIComponent(dynPropArr[0]) : "";
            jvx_6656684427dbb.DYSegmentName = dynPropArr[1] ? decodeURIComponent(dynPropArr[1]) : "";
			jvx_6656684427dbb.eventServerParams = dynPropArr[2] ? decodeURIComponent(dynPropArr[2]) : "";   //Getting trafficker events output from layout_renderer_html.php
            //this.fireCREvent();
			jvx_6656684427dbbdyPropsACK = true;
         },
			...
			if(doFireCreativeLoadEvt){
				...
				jvx_6656684427dbb.recordEvent(72, 'es_et=' + creativeLoadElapseTime);
			}
		},
		onLoadEventHandler:function(event){
                    ...
                    if(jvx_6656684427dbbSafeFrameSupported){
                        var getReportingKeyFromCookie = false,
                            ...
                            if(getReportingKeyFromCookie){
                                jvx_6656684427dbb.sendCookieRequest('jvx_6656684427dbb'+"_dynamicProperties");
                            }
                    }
                											}	
		},
			...
			baseContainerNode.appendChild(baseNode);
								...
								jvx_6656684427dbb.layoutLoadedTime = Date.now();
			...
			var executeCallBack = function(transitionEvt){
					...
					if(ref.expansionOpen){
						...
						if(ref.openExpAgain) ref.openExpAgain = false; 
												...
												if(jvx_6656684427dbbSafeFrameSupported && transitionEvt){
							try{
								$sf.ext.collapse();
															}catch(e){}
						}
		...
		getIBusterFrameInfo:function(){
			if(typeof(jivoxIBusterAdObject) != "undefined"){
				...
				for(var i=0; i < iFramesLen; i++){
					if(hostIframes[i].contentWindow == jivoxIBusterAdObject["jvx_6656684427dbb"].foreignWindow){
...
//				e2Node.style.margin = mt + ' 0 0 ' + ml;
		...
		openWindow:function(url){
							if(jvx_6656684427dbbNonMraidTagInMraidEnv){
					mraid.open(url);
				} else {
					window.open(url);
				}
		...
		openClickThrough:function(event={}){
				...
				console.log("Unexpected error occured while fetching Event obj " + err);
											...
											this.raiseCMacroFlag = true;
										
				if(!this.raiseCMacroFlag){
					var eventRecordParams = "c_cx="+clickEventDetails.clientX+"/c_cy="+clickEventDetails.clientY+"/c_ct="+clickEventDetails.ct+"/c_tv="+(clickEventDetails.ct-jvx_6656684427dbb.layoutLoadedTime);
					this.recordEvent(1, eventRecordParams);
				}else{
					var eventRecordParams = "es_clkMac_1/c_cx="+clickEventDetails.clientX+"/c_cy="+clickEventDetails.clientY+"/c_ct="+clickEventDetails.ct+"/c_tv="+(clickEventDetails.ct-jvx_6656684427dbb.layoutLoadedTime);
					this.recordEvent(1, eventRecordParams);
				}
									if(!adInVPForOneSec){
												this.recordEvent(73);
				...
				openExpansion:function(autoExpand, skipOpenEvent){
						...
						if(!jvx_6656684427dbbCanBustFromFrame || jvx_6656684427dbbInNestedFrame){
				return;
			}
		...
		recordEvent:function(eventId,addnlParams){
											if( !jvx_6656684427dbbdyPropsACK && (eventId==48 || eventId==49 || eventId==1)){
					addnlParams = (addnlParams) ? addnlParams + "/c_actualEventId="+eventId : "c_actualEventId="+eventId;
					eventId = 78;
				}
								var eventReportingParams = '';
											eventReportingParams = "224758/2002/177388/2637e5c7c09080/20/jvxSId_1716938820.8253";		
												var rand = Math.random();
				var imgEl = document.createElement("img");
                var tempURL = "";
                var eventReportingURL = "https://evs.jivox.com/trk" + "/" + eventId + "/" + eventReportingParams;
                eventReportingURL += "/es_pId_3cbb2b9c";
                tempURL += "";
                                tempURL += "";
                									if(jvx_6656684427dbb.eventServerParams != "" && jvx_6656684427dbb.eventServerParams != undefined){
						tempURL += "/" + jvx_6656684427dbb.eventServerParams;
					}
				                tempURL = addnlParams ? (tempURL + "/" + addnlParams) : tempURL;
									tempURL += "/bDim="+ "320x50";
								tempURL = tempURL + "/jvxRandom=" + rand;
				//eventReportingURL = (eventId == 1) ? (eventReportingURL + ";https://www.t-mobile.com/") : eventReportingURL;
									tempURL += "/es_cgName="+jvx_6656684427dbb.DYReportingKey;
                    if(jvx_6656684427dbb.DYSegmentName != ""){
                        tempURL += "/es_segName="+jvx_6656684427dbb.DYSegmentName;
                    }
		...
		recordDwellEvent:function(eventId, elapsedTime){
			...
			this.DwellTimer = null;
						...
						var cNode = createDivNode(this.adUnitContainerId);
			            												document.write('<div id="jvxScrRefNode_6656684427dbb" style="display:none;"></div>');	
				var jvxPNode = '';
				var jvxNode = getElById("jvxScrRefNode_6656684427dbb");
			...
			if(bestFitBase) {
				...
				if(this.baseHasLayout == 1) {
					this.baseType = 'html';
					this.baseURL = 'https://as.jivox.com/unit/layout_renderer.php?cbName=__IntegralAS_b561874ffc254b74eaab2e944b58d4f6_4833&es_pId=3cbb2b9c&showAdChoices=0&isDynamic=1&campaignId=177388&ts_pId=3cbb2b9c&siteId=2637e5c7c09080&dspId=a7b0f97e-f637-4081-9190-a5ae87cce1a9&bDim=320x50&creativeUnitType=20&jvxVer=2&bUnitId=2002&us_privacy=%24%7BUS_PRIVACY%7D&gdpr_consent=&gdpr=0&r=%7B%25random%7D&cMacro=%7B%25short_redirect%7D&vfwctx=eyJjIjoiYWxsLTEifQ&gdpr_pd=1&format=jsonp&objectName=jvx_6656684427dbb';
											this.baseURL += '&jvxSessionId=1716938820.8253';
			...
			if(bestFitExpand) {
				...
				if(this.expansionHasLayout == 1) {
					this.expansionType = 'html';
										var adUnitIdStr = '';
										this.expansionURL = 'https://as.jivox.com/unit/layout_renderer.php?cbName=__IntegralAS_b561874ffc254b74eaab2e944b58d4f6_4833&es_pId=3cbb2b9c&showAdChoices=0&isDynamic=1&campaignId=177388&ts_pId=3cbb2b9c&siteId=2637e5c7c09080&dspId=a7b0f97e-f637-4081-9190-a5ae87cce1a9&bDim=320x50&creativeUnitType=20&jvxVer=2&&us_privacy=%24%7BUS_PRIVACY%7D&gdpr_consent=&gdpr=0&r=%7B%25random%7D&cMacro=%7B%25short_redirect%7D&vfwctx=eyJjIjoiYWxsLTEifQ&gdpr_pd=1&format=jsonp&objectName=jvx_6656684427dbb'+adUnitIdStr;
											this.expansionURL += '&jvxSessionId=1716938820.8253';
		...
		listenerCallback:function(event) {
			jvx_6656684427dbb.eventController(event); 
		},
...
var jvx_6656684427dbb = new CreativeUnit("_6656684427dbb");
jvx_6656684427dbb.DYReportingKey = "";
jvx_6656684427dbb.DYSegmentName = "";
jvx_6656684427dbb.eventServerParams = "";
	if(jvx_6656684427dbbNonMraidTagInMraidEnv){
				jvx_6656684427dbb.recordEvent(75);
	}
    jvx_6656684427dbb.init();
