...
export default function SlideOutMenu({screenSize, currentPage, updateCurrentPage, isVisible, updateIsShowingSlideOutMenu}) {
  ...
  return (
    ...
    <div className={`h-full w-[150px] bg-light7 flex flex-col px-4 text-black pt-[150px]  mobileLandscape:pt-0 fixed right-0 z-40 border-l-[1px] border-black transform transition-transform duration-200 ${isVisible ? 'translate-x-0' : 'translate-x-full' }`}>
        ...
        <div className='border-b-[1px] border-black w-full'>
            <SlideOutMenuLink title={"Media"} to={"/media"} iconName={"microphone-1"} tabColor={'#3E8483'} tabClass={'bgTeal'} currentPage={currentPage} updateCurrentPage={updateCurrentPage} updateIsShowingSlideOutMenu={updateIsShowingSlideOutMenu}/>
        </div>
        {/* <div className='border-b-[1px] border-black w-full'>
